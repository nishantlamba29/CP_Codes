...
inline void n_lamba_29(){
    ...
    vvec dist(n+1, vec(n+1, inf)), mat(n+1, vec(n+1));
    ...
    for(auto &node : v){
        ...
        for(int j=1;j<=n;j++) {
            if(vis[j]){
                dist[node][j] = mat[node][j];
                dist[j][node] = mat[j][node];
            }
        }
        ...
        for(int k=1;k<=n;k++){
            for(int j=1;j<=n;j++){
                if(vis[j] && vis[k])
                dist[node][j] = min(dist[node][j], dist[node][k] + dist[k][j]);
            }
        }
        ...
        for(int k=1;k<=n;k++){
            for(int j=1;j<=n;j++){
                if(vis[j] && vis[k])
                dist[j][node] = min(dist[j][node], dist[j][k] + dist[k][node]);
            }
        }
