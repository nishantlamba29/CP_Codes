...
inline void n_lamba_29(){
    ...
    rep(m){
        ...
        cin>>a>>d>>k;
        for(int j=1;j<=k;j++){
            if(ds.union_set(a, a+j*d))
            ds.union_set(a, a+j*d);
            // else {
            //     int par = ds.find_par(a);
            //     st.insert({a,d,k, par});
            //     break;
            // }
        }
    ...
    // for(auto &i : st){
    //     int a = i[0], d = i[1], k = i[2], par = i[3];
    //     for(int j=1;j<=k;j++) ds.parent[a+j*d] = par;
    // }
    int cnt = 0;
    for(int i=1;i<=n;i++){
        if(ds.find_par(i) == i) cnt++;
    }
    cout<<cnt<<"\n";
}
