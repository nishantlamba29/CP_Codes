...
inline void n_lamba_29(){
    ...
    for(int i=0;i<n/2;i++){
        for(int j=0;j<n/2;j++){
            char a = max({v[i][j], v[n-j-1][i], v[j][n-i-1], v[n-i-1][n-j-1]});
            ans += a-v[i][j];  v[i][j] = a;
            ans += a-v[n-j-1][i];  v[n-j-1][i] = a;
            ans += a-v[j][n-i-1];  v[j][n-i-1] = a;
            ans += a-v[n-i-1][n-j-1];  v[n-i-1][n-j-1] = a;
        }
    }
