...
inline void n_lamba_29(){
    ...
    auto check = [&](int mid){
        ...
        int t2 = 0;
        for(int i=1;i<=n;i++){
            if(mid >= f[i]) t1 += (mid-f[i])/2; // t1 task 2 hour wale kr sakta hu
            else t2 += f[i]-mid; // t2 task 2 hour wale krne h
            // 2 hour wale nhi , jo bch gye vo task
        }
        return t2 <= t1; // krne h <= kr sakta hu
    };
