...
inline void n_lamba_29(){
    ...
    vec a(n), b(n);
    set<int> st;
    rep(n) cin>>a[i], st.insert(a[i]);
    rep(n) cin>>b[i], st.insert(b[i]);
    sort(all(a));
    sort(all(b));
    int ans = 0;
    for(auto i : st){
        int ind1 = lower_bound(all(a), i) - a.begin();
        int ind2 = lower_bound(all(b), i) - b.begin();
        if(abs(ind2-ind1) <= k){
            ans = max(ans, i*(n-min(ind2, ind1)));
        }
    }
    print(ans);
